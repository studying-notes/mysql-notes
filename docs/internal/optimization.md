---
date: 2020-10-23 15:55:15+08:00  # 创建日期
author: "Rustle Karl"  # 作者

title: "MySQL 查询优化"  # 文章标题
url:  "posts/mysql/docs/internal/optimization"  # 设置网页永久链接
...
- 尽量避免在 where 子句中对字段进行 null 值判断，否则将导致引擎放弃使用索引而进行全表扫描。可以设置默认值 0，确保表中列没有 null 值。
- 尽量避免在 where 子句中使用 or 来连接条件，否则将导致引擎放弃使用索引而进行全表扫描，可以用 union 代替。
- 模糊查询 `% %` 将导致全表扫描，若要提高效率，可以考虑全文检索。
- 慎用 `in` 和 `not in`，否则会导致全表扫描。
- 如果在 where 子句中使用参数，也会导致全表扫描。因为 SQL 只有在运行时才会解析局部变量，但优化程序不能将访问计划的选择推迟到运行时；它必须在编译时进行选择。然而，如果在编译时建立访问计划，变量的值还是未知的，因而无法作为索引选择的输入项。
- 应尽量避免在 where 子句中对左侧字段进行表达式操作，这将导致引擎放弃使用索引而进行全表扫描。
- 应尽量避免在 where 子句中对左侧字段进行函数操作，这将导致引擎放弃使用索引而进行全表扫描。
